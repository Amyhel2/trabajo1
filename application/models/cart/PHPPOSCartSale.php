	...
	*/
	...
	private function get_number_of_free_or_discount_items($rule,$params)
	{
		...
		{	
			...
			if($this->is_price_rule_discount_item_line($params['line']))
			{
				...
				$discount_item_quantity = $line_item->quantity;
			}
			//regular item line
			else 
			{
				$line_item = $this->get_item($params['line']);
				$reg_item_quantity = $line_item->quantity;
	...
	public function select_customer($customer_id)
	{
		...
		if ($CI->Customer->exists($customer_id))
		{
			...
			if ($CI->config->item('validate_location_id_of_customer_when_adding_to_sale'))
			{
				if ($customer_info->location_id && $customer_info->location_id!=$CI->Employee->get_logged_in_employee_current_location_id())
				{
					$data['error']=lang('sales_unable_to_add_customer_not_part_of_this_location');
					return $data;
				}
			}
