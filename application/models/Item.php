  ...
  */
  function add_variation_data(&$suggestions, $search, $price_field)
  {
	...
	*/
	...
	function delete_image($image_id, $item_id)
	{
		...
		$this->db->where('image_id', $image_id);
		$query = $this->db->select('COUNT(id) as count')->get('item_images');
		$result = $query->row();
		$count_image_ref = $result->count;

		if($count_image_ref == 0){
	...
	public function delete_all_images($item_id)
	{
		...
		{
			$this->delete_image($row['image_id'], $item_id);
		}
	}

	public function update_multiple_item_image($item_ids, $select_inventory, $image_ids, $replace = 0){
		if($select_inventory)
		{
			if ($this->is_empty_search())
			{
				...
				$query = $this->db->select_max('id')->get('item_images');
				$result = $query->row();
				$offset_id = 0;
				if($result){
					$offset_id = $result->id;
				}
				if($replace == 1){//replace all
					$this->db->update('items',array('main_image_id' => NULL));
					create_and_execute_large_update_query_item_images_remove(array(), $offset_id);
				}

				$this->db->from('items');
				foreach($this->db->get()->result_array() as $row){
					foreach($image_ids as $image_id){
						$this->db->insert('item_images', array('item_id' => $row['item_id'], 'image_id' => $image_id));
					}
				}

				if($replace == 1){//replace all
					$this->db->update('items', array('main_image_id' => $image_ids[count($image_ids) - 1]));
				}else{
					$this->db->where('main_image_id is NULL');
					$this->db->update('items', array('main_image_id' => $image_ids[count($image_ids) - 1]));
				}
			} else {
				$this->load->helper('database');
				...
				if($replace){
					create_and_execute_large_update_query_item_images_remove($item_ids, $offset_id);
				}
				...
				return create_and_execute_large_update_query_items_main_image_set($item_ids, $image_ids[count($image_ids) - 1], $replace);
			}
		}
		else
		{
