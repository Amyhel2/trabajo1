...
class Woo extends Ecom
{
	...
	function get_categories($use_cache = TRUE)
	{
		...
		{
			$categories[] = array('name' => html_entity_decode($category['name']),'description' => $category['description'], 'id' => $category['id'], 'parent' => $category['parent'], 'image' => isset($category['image']['src']) ? $category['image']['src'] : FALSE);
		}
		...
		foreach($this->categories_result as $cat_path => $woo_cat_id)
		{
			...
			$category_description = $categories[$index]['description'];
			...
			{
				$this->Category->link_description($categories_indexed_by_name[mystrtoupper($cat_path)], $category_description);
			}
