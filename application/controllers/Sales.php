...
class Sales extends Secure_area
{
  ...
  function cancel_sale()
  {
	...
	//FUNCIONES PARA LA API FACTURACION

	public function invoice_service()
	{
    	$this->load->helper('form');
    	$this->load->view('sales/listar_facturas');
	}

	public function generate_invoice($sale_id)
	{
		$this->load->model('Sale');
		$this->load->model('Customer');
		...
		$maestro = [
			...
			"cmp" => "",
			"tipodoc" => "1",
			"mail" => "metasoft.bolivia@gmail.com",
			"nrocafc" => "",   
			"fcafc" => "",
			"hcafc" => ""
		];
		...
		curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($factura_data));
		...
		if (trim($response) === "true") {
			...
			// 4. LISTAR FACTURAS DEL DÍA PARA ESTE NIT
			...
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json']);
			curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($listar_request));
			$listar_response = curl_exec($ch);
			curl_close($ch);
	
			$facturas = json_decode($listar_response, true)['facturas'] ?? [];
	
			// Buscar la última factura del cliente actual (por NIT)
			$idfac = null;
			foreach (array_reverse($facturas) as $factura) {
				if ($factura['numeroDocumento'] === $customer_info->account_number) {
					$idfac = $factura['id'];
					break;
				}
			}
			...
			if ($idfac) {
				...
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
				curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json']);
				curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($pdf_request));
				$pdf_response = curl_exec($ch);
				curl_close($ch);
	
				if (trim($pdf_response) === "true") {
					$this->session->set_flashdata('message', '✅ Factura generada y PDF creado (ID: ' . $idfac . ')');
				} else {
					$this->session->set_flashdata('message', '⚠️ Factura generada pero error al crear PDF');
				}
			} else {
				$this->session->set_flashdata('message', '⚠️ Factura creada pero no se encontró el ID para el cliente');
			}
	
		} else {
			$this->session->set_flashdata('message', '❌ Error al procesar factura: ' . $response);
		}
	...
	public function list_invoices()
{
    ...
    $this->load->view('sales/list_invoices', [
        'facturas' => $facturas,
        'fechainicio' => $start_date,
        'fechafin' => $end_date
    ]);
