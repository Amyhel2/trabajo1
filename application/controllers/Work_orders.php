...
class Work_orders extends Secure_area
{
	...
	*/
	...
	private function _get_work_order_data($work_order_id)
	{
		...
		$data['cart'] = PHPPOSCartSale::get_instance_from_sale_id($work_order_info['sale_id']);
	...
	function edit_sale_item_discount_percent($sale_id,$item_id,$item_variation_id=false, $line, $is_item_kit = false)
	{
		if($is_item_kit){
			$item_name 	= $this->Item_kit->get_info($item_id)->name;
			$sale_item 	= $this->Sale->get_sale_item_kits($sale_id,$line)->row();
			$oldvalue 	= $sale_item->discount_percent ?? '0.00';
		}else{
			$item_name 	= $this->Item->get_info($item_id)->name;
			$sale_item 	= $this->Sale->get_sale_item($sale_id,$item_id,$line);
			$oldvalue 	= $sale_item->discount_percent;
		}

		if($item_variation_id){
			$item_id = $item_id.'#'.$item_variation_id;
		}
		$discount_percent = $this->input->post("value");
		$this->Sale->sale_item_discount_update($sale_id,$item_id,$line,$discount_percent,$is_item_kit);
		...
		if ($work_order_id)
		{
			$this->Work_order->log_activity($work_order_id,$item_name.' [field]discount_percent[/field] '.lang('common_changed').' '.lang('common_from').' [oldvalue]'.$oldvalue.'[/oldvalue] '.lang('common_to').' [newvalue]'.$discount_percent.'[/newvalue]');
		}
	}
